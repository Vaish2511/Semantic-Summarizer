...
class TextProcessor:
    ...
    def clean_text(text):
        ...
        # drop everything after Acknowledgment or References
        lower = txt.lower()
    ...
    def remove_redundant_sentences(text: str):
        # Tokenizing sentences using regex (similar to nltk.sent_tokenize)
    ...
    def smart_split_sentences(text, max_sentences=5):
        # Tokenizing sentences with regex
        sentences = re.split(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s', text)
        chunks = []
    ...
    def word_tokenize(text):
        # Tokenizing words with regex (similar to nltk.word_tokenize)
...
class Summarizer:
    ...
    def summarize_text_few_shot(self, text):
        # Preprocess and clean text (remove redundancy and unwanted sections)
        ...
        # After summarizing all chunks, join the summaries
    ...
    def extractive_top_k(self, text: str, k: int=3):
        """ Embed sentences & document, rank by cosine similarity to doc, return top k sentences. """
        sentences = re.split(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s', text)  # Tokenize sentences using regex
        if len(sentences) <= k:
            return sentences
